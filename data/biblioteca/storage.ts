...
export async function createStorage() {
    try {
        ...
        if(automatasJson == null) {
            ...
            await AsyncStorage.setItem('automatas', JSON.stringify([sumaBinariaEntry]))
...
export async function addAutomata(automata: Automata, posicion: number) {
    try {
      ...
      await AsyncStorage.setItem(posicion.toString(), automataJson);

      const automatasEntries = getAutomatas()
    } catch (e) {
      console.log("Ocurrio un error al guardar el automata");
    }
};
...
export async function setAutomatas(automatasEntries: AutomataEntry[]) {
    try {
      const automatasEntriesJson = JSON.stringify(automatasEntries);
    
      const mergedAutomatas = await AsyncStorage.mergeItem('automatas', JSON.stringify(automatasEntries))

      if(mergedAutomatas == null) return null
      else return getAutomatas()
...
export async function getAutomata(indice: number){
    try {
      ...
      if(automatasJson != null)
        return JSON.parse(automatasJson)
